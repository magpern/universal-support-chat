 ...
 */
function universal_support_chat_collect_markdown_files( string $dir ): array {
	$files = array();
 ...
 * @return array<int, string>
 */
function universal_support_chat_extract_relative_links( string $content ): array {
	$links = array();
 ...
 */
function universal_support_chat_check_doc_links( array $paths ): int {
	$files = array();

	foreach ( $paths as $path ) {
		if ( is_dir( $path ) ) {
			$files = array_merge( $files, universal_support_chat_collect_markdown_files( $path ) );
			continue;
		}
	...
	foreach ( $files as $file ) {
		...
		foreach ( universal_support_chat_extract_relative_links( $content ) as $link ) {
...
$universal_support_chat_doc_link_paths = array_slice( $argv, 1 );

if ( array() === $universal_support_chat_doc_link_paths ) {
	fwrite( STDERR, "Usage: php bin/check-doc-links.php <path> [<path> ...]\n" );
	exit( 1 );
}

exit( universal_support_chat_check_doc_links( $universal_support_chat_doc_link_paths ) );
