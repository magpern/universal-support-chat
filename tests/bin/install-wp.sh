...
#
# On success, writes /tmp/usc-wp-env.sh exporting WP_TESTS_DIR and
# WP_CORE_DIR (and USC_TEST_WC_ACTIVE=1 if a WooCommerce version was given)
...

{
    ...
    if [ -n "$WC_VERSION" ]; then
        echo "export USC_TEST_WC_ACTIVE=1"
        echo "export USC_TEST_WC_VERSION='${WC_VERSION}'"
    fi
} > "$ENV_FILE"
