...
class CredentialVault {
	...
	private const ENVELOPE_VERSION = 'usc1:';
	...
	private function resolve_key(): array {
		...
		if ( null !== $explicit ) {
			...
			if ( null === $key ) {
				throw new CredentialUnavailableException( 'UNIVERSAL_SUPPORT_CHAT_CREDENTIAL_KEY is defined but malformed.' );
			}
	...
	private function resolve_key_for_source( int $source ): string {
		if ( self::KEY_SOURCE_EXPLICIT_CONSTANT === $source ) {
			...
			if ( null === $explicit ) {
				throw new CredentialUnavailableException( 'UNIVERSAL_SUPPORT_CHAT_CREDENTIAL_KEY is not defined.' );
			}

			$key = $this->decode_explicit_key( $explicit );

			if ( null === $key ) {
				throw new CredentialUnavailableException( 'UNIVERSAL_SUPPORT_CHAT_CREDENTIAL_KEY is malformed.' );
			}
	...
	protected function explicit_key_constant(): ?string {
		return defined( 'UNIVERSAL_SUPPORT_CHAT_CREDENTIAL_KEY' ) ? UNIVERSAL_SUPPORT_CHAT_CREDENTIAL_KEY : null;
	}
