...
class ConversationRepository {
	...
	/**
	 * Inserts a conversation row with explicit historical field values,
	 * preserving legacy timestamps/status/assignment instead of defaulting
	 * them to "now"/`new` the way `create()` always does. Used only by the
	 * SC-M03 legacy migration engine (`Migration\PhaseABackfillService`) —
	 * never by any live conversation-start path. Always generates a fresh
	 * `conversation_uuid`; the legacy conversation's own UUID is never
	 * reused as a Support Chat identifier (ADR-0008 §2, sc-m03-wp3-wp4
	 * plan §4.1).
	 *
	 * @param int         $owner_user_id         Verified-equivalent visitor owner (PO decision record item 1).
	 * @param string      $status                Copied 1:1 from the legacy row.
	 * @param int|null    $assigned_operator_id  Copied 1:1 from the legacy row (PO decision record item 2).
	 * @param string      $start_idempotency_key A pre-derived, collision-safe target key (never the raw legacy value).
	 * @param string      $created_at            Preserved legacy `created_at`.
	 * @param string      $updated_at            Preserved legacy `updated_at`.
	 * @param string|null $resolved_at           Preserved legacy `resolved_at`.
	 * @param string|null $expires_at            Preserved legacy `expires_at`.
	 *
	 * @return Conversation|null Null if the schema is unavailable or the write failed
	 *                           (including a unique-constraint collision on the derived
	 *                           `start_idempotency_key`, which the caller must treat as a
	 *                           typed migration failure, never silently retried with a new value).
	 */
	...
	/**
	 * Sets `assignee_last_seen_message_id` directly to an already-resolved
	 * target message id (or `null`) — used only by the legacy migration
	 * engine after remapping the legacy value through
	 * `Migration\LegacyMigrationMessageMapRepository` (sc-m03-wp3-wp4 plan §4.1).
	 *
	 * @param int      $id                            Conversation primary key.
	 * @param int|null $assignee_last_seen_message_id Already-remapped target message id, or null.
	 */
	public function set_assignee_last_seen_message_id( int $id, ?int $assignee_last_seen_message_id ): bool {
		if ( ! $this->schema_health->is_available() ) {
			return false;
		}
		...
		$updated = $wpdb->update(
			$table,
			array( 'assignee_last_seen_message_id' => $assignee_last_seen_message_id ),
			array( 'id' => $id ),
			array( '%d' ),
			array( '%d' )
		);
