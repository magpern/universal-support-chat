 ...
 *
 * The SC-M03 final-cutover handoff-provenance path (ADR-0010 §4 —
 * `source_bot_id`/`source_update_id` request fields, a wrapping transaction,
 * and a `legacy_handoff_map` write) was RETIRED here (ADR-0013): Universal
 * Telegram ADR-0044 made that plugin transport/adapter-only, so no
 * cutover-replay traffic exists. Ordinary adapter traffic never populated
 * those fields, so this retirement is behaviour-preserving for every live
 * call.
 */
final class ContractOperationDispatcher {
	...
	private function ingest_operator_reply( string $peer_id, array $body ): array {
		...
		if ( null !== $this->dispatch ) {
			// Loop prevention (ADR-0012): a reply that arrived *from*
			// Telegram must never be mirrored back out.
