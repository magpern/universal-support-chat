...
| [sc-m03-final-cutover-dev-rehearsal-plan-v2.md](sc-m03-final-cutover-dev-rehearsal-plan-v2.md) | SC-M03 final-cutover disposable DEV rehearsal — Support Chat companion **v2** (current; supersedes v1; primary runbook in Universal Telegram). Pins the current origin/main heads UT `33b042f` / SC `2000eaf` (src+tests identical to the F1 correction; only docs added since the F1 closure `32f17ea` / `5d81b5b`); revises only the F1-invalidated portions of v1 (wire identity = the Support Chat conversation UUID; Run 1 handoff fixture/assertions with real distinct `binding_uuid`; exhaustive fail-closed classifier `unresolved_case_reference` / `handoff_rejected` referenced by Runs 2/3; new Run 1 F1-correction gate; new Run 3 fail-closed incident scenarios); all v1 safety boundaries, evidence/redaction/teardown requirements, the Tier 1/Tier 2 distinction, and blockers B1–B5 carried forward. **Authorizes nothing** — a Tier 1 re-attempt needs the **proposed, unsigned** Approval A addendum; Tier 2 blocked on B1/B2 and pending Approval B; no rehearsal has run under v2) |
