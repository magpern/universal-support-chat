...
- `LegacyFieldMap.php` — the complete, CI-enforced disposition registry for every real Universal Telegram `conversations`/`conversation_messages`/`conversation_notes` column (26 + 11 + 5 = 42 columns), matching plan §4.1 exactly, including the field-mapping correction this closure made: `owner_active_slot` (a generated-index column) was present in the plan's own field-mapping table but missing from the first draft of this registry — caught by `Interop\SchemaInventoryTest` itself against Universal Telegram's real schema before this closure was written, not discovered later.
...
- `LegacyFieldMap::REGISTRY` covers all 42 real Universal Telegram columns (`tests/unit/Migration/LegacyFieldMapTest.php`); `Interop\SchemaInventoryTest` independently confirms this against Universal Telegram's real, live schema, not a static snapshot.
...
|---|---|---|
| Unit | `bin/docker/test-unit.sh` | 82 tests, 733 assertions — OK (59 pre-existing + 23 new) |
| Integration (WP 6.9 / PHP 8.1) | `bin/docker/test-integration-wp-only.sh --wp-version=6.9 --php-version=8.1` | 93 tests, 371 assertions — OK (70 pre-existing + 23 new) |
| Integration (WP 7.1 / PHP 8.3) | `bin/docker/test-integration-wp-only.sh --wp-version=7.1 --php-version=8.3` | 93 tests, 371 assertions — OK |
| **Interop (dual-plugin, WP 6.9 / PHP 8.1)** | `bin/docker/test-integration-interop.sh --wp-version=6.9 --php-version=8.1` | 6 tests, 25 assertions — OK, against Universal Telegram's real, merged `main` (`5d16119`) |
...
New test files: `tests/unit/Migration/{IdempotencyKeyDeriverTest,DefaultDenyQuiescenceStateProviderTest,LegacyFieldMapTest,LegacyMigrationValidatorTest}.php`; `tests/integration/Migration/{PhaseABackfillServiceTest,PhaseBReconciliationServiceTest}.php` (12 + 5 tests), `tests/integration/Migration/Cli/LegacyMigrateCommandTest.php` (6 tests, with a minimal test-only `\WP_CLI` stub since this environment never loads the real WP-CLI binary); `tests/integration/Migration/Support/{FakeLegacyExportClient,FakeQuiescenceStateProvider}.php` (test doubles, not production code); `tests/integration/Interop/{SchemaInventoryTest,LegacyExportClientIntegrationTest}.php` (6 tests, real dual-plugin).
