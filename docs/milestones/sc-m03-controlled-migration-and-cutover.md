...
> **Retired (planning-only note, ADR-0013).** Universal Telegram ADR-0044 made that plugin transport/adapter-only and closed this track. The Support Chat legacy-migration / final-cutover **engine, its `wp universal-support-chat legacy-migrate` / `legacy-bind` WP-CLI commands, and the `ContractOperationDispatcher` handoff-provenance path have been removed from `src/`** — see [`docs/closure/sc-m03-engine-retirement-closure.md`](../closure/sc-m03-engine-retirement-closure.md) and [ADR-0013](../adr/0013-retirement-of-obsolete-sc-m03-migration-cutover-engine.md). Schema version is unchanged (`db_version` 12; steps 9–11 are inert no-ops); installed historical migration tables/data and all SC-M03 documents below are preserved as historical evidence. Support Chat remains the sole conversation system of record; ADR-0012 Telegram dispatch is unaffected. Everything below is the historical record of the retired track.
...
**F1 resolution — implementation ACCEPTED 2026-08-27:** [ADR-0011](../adr/0011-cutover-channel-case-ref-is-support-chat-conversation-uuid.md) (**Accepted**) amends ADR-0010 §4: `channel_case_ref` is unambiguously this repository's conversation/case UUID — `resolve_conversation()` and `dispatch_with_provenance()` behaviour are already correct, only comments contradict it (corrections C1–C4). Universal Telegram ADR-0043 owns the adapter-side wire change (send `support_conversation_uuid()`, not `binding_uuid()`) and the fail-closed classification of an unresolvable ref (new closed incident `unresolved_case_reference`). No SC-side binding→conversation resolver is added (rejected unless separately designed later). Remediation plan: [`docs/plans/sc-m03-final-cutover-f1-channel-case-ref-remediation-plan-v1.md`](../plans/sc-m03-final-cutover-f1-channel-case-ref-remediation-plan-v1.md) — comment corrections only; no schema, no `db_version` bump. Identifier collapse (option (c)) rejected by Product Owner direction.

**F1 implementation and closure MERGED 2026-08-27** ([decision record](../decisions/sc-m03-final-cutover-dev-rehearsal-po-decisions.md) Addendum B): universal-support-chat #26 `9144cb1e2362c2be8d4c74f1461bba7ffe236575` (comment corrections C1–C4 only) / closure #27 `5d81b5b7795ee50f3a79e535a483d7677b36d1c0`; universal-telegram #53 `7d4cc4fecb97f862721cea0fec427ade26b46ea7` / closure #54 `32f17ea904a33cdd1f9b0225ba9638f95a09d883`. Closure: [F1 implementation closure](../closure/sc-m03-final-cutover-f1-identity-correction-implementation-closure.md). The real dual-plugin interop suite passed **OK (47 tests, 722 assertions)** on both supported WP/PHP variants post-merge. No schema / `universal_support_chat_db_version` / Contract-operation change; no SC-side resolver.

**Planning-only:** [DEV rehearsal runbook **v2**](../plans/sc-m03-final-cutover-dev-rehearsal-plan-v2.md) (primary in Universal Telegram) supersedes v1, pins the immutable Product-Owner-approved Tier 1 execution baselines UT `6eed0228286e84b4e56e0119f242b483f138a58e` / SC `4f833c3344c3cff2adcc0227f93832c0c3a4427a` (operators fetch origin, verify these exact commits exist, and check them out before execution; runtime trees byte-identical to the F1 implementation commits `7d4cc4f` / `9144cb1`, documentation only added since; future documentation merges must not alter the baseline without a new PO approval), and revises only the F1-invalidated portions (wire identity, Run 1 handoff fixture/assertions with a real distinct `binding_uuid`, the exhaustive fail-closed classifier referenced by Runs 2/3, a new Run 1 F1-correction gate, new Run 3 fail-closed incident scenarios); all v1 safety boundaries, evidence/redaction/teardown requirements, the Tier 1/Tier 2 distinction, and blockers B1–B5 carried forward. F1 is **no longer a code blocker**. The Approval A addendum (Universal Telegram `docs/closure/sc-m03-final-cutover-dev-rehearsal-tier1-approval-addendum.md`; verbatim text and acceptance also in the decision record, Addendum B / Addendum C) is **RECORDED / Product Owner accepted 2026-08-28**, and **the single authorised Tier 1 re-attempt was executed on 2026-08-28 and PASSED** on both supported WP/PHP variants at the immutable execution baseline SHAs (decision record Addendum D; closure [`docs/closure/sc-m03-final-cutover-dev-rehearsal-tier1-reattempt-closure.md`](../closure/sc-m03-final-cutover-dev-rehearsal-tier1-reattempt-closure.md) — dual-plugin interop `OK (47 tests, 722 assertions)` each variant; F1-correction gate held with a real `legacy-bind` binding; fail-closed classifier confirmed; zero Telegram traffic; teardown proven). **Tier 1 is complete; the addendum’s one-time authorisation is consumed.** The **Tier 2 prerequisite design is now frozen** — [Tier 2 prerequisites plan v1](../plans/sc-m03-final-cutover-tier2-dev-rehearsal-prerequisites-plan-v1.md) (primary in Universal Telegram; decision record Addendum E) specifies B1 (isolated instance + verification gate), B2 (dedicated non-production bot/supergroup/topics + verification gate), the Tier 2 operator sequence, and a **proposed, unsigned Approval B**. It provisions nothing and authorizes nothing. Tier 2 stays blocked on B1 and B2 being provisioned and independently verified, then Approval B; a second Tier 1 attempt needs a new Product Owner approval.
