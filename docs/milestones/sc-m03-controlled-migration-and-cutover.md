...
**Planning-only:** [DEV rehearsal runbook **v2**](../plans/sc-m03-final-cutover-dev-rehearsal-plan-v2.md) (primary in Universal Telegram) supersedes v1, pins the current origin/main heads UT `33b042f` / SC `2000eaf` (src+tests identical to the F1 correction; only docs added since the F1 closure `32f17ea` / `5d81b5b`), and revises only the F1-invalidated portions (wire identity, Run 1 handoff fixture/assertions with a real distinct `binding_uuid`, the exhaustive fail-closed classifier referenced by Runs 2/3, a new Run 1 F1-correction gate, new Run 3 fail-closed incident scenarios); all v1 safety boundaries, evidence/redaction/teardown requirements, the Tier 1/Tier 2 distinction, and blockers B1–B5 carried forward. F1 is **no longer a code blocker**. **No rehearsal has run under v2.** A Tier 1 re-attempt requires the **proposed, unsigned** Approval A addendum (Universal Telegram `docs/closure/sc-m03-final-cutover-dev-rehearsal-tier1-approval-addendum.md`; verbatim text also in the decision record, Addendum B) — the original Approval A was consumed by the halted first attempt. Tier 2 stays blocked on B1 and B2 and pending Approval B.
