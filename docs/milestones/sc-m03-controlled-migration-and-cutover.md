...
**F1 resolution — planning-only (2026-08-27):** [ADR-0011](../adr/0011-cutover-channel-case-ref-is-support-chat-conversation-uuid.md) (Proposed) corrects ADR-0010 §4 so `channel_case_ref` is unambiguously this repository's `conversation_uuid` — `resolve_conversation()` behaviour is already correct; only comments contradict it. Universal Telegram ADR-0043 owns the adapter-side wire change (send `support_conversation_uuid()`, not `binding_uuid()`). Remediation plan: [`docs/plans/sc-m03-final-cutover-f1-channel-case-ref-remediation-plan-v1.md`](../plans/sc-m03-final-cutover-f1-channel-case-ref-remediation-plan-v1.md) (comment corrections only; no schema, no `db_version` bump). Identifier collapse (option (c)) is rejected by Product Owner direction. Implementation gated on Product Owner acceptance of both ADRs; no Tier 1 / Tier 2 acceptance record exists.
