...
The next available number for any future ADR is **0018**.
...
- [ADR-0017 — Support availability authority, and honest offline / offline-ticket behaviour](0017-support-availability-authority-and-honest-offline-behaviour.md) — **Proposed** (SC-M06; companion plan `docs/plans/sc-m06-support-availability-and-offline-tickets-plan-v2.md`). Support Chat is the sole availability authority; site-timezone evaluation; precedence `manual override → date exception → weekly schedule → fail-safe unavailable`; visitor-copy honesty (no untrue online / response-time claim); offline ticket = existing authenticated conversation transitioned to `waiting_for_operator` in one transaction via a new `new → waiting_for_operator` map edge; no Telegram mechanism, no AI, no schema / `db_version` / capability change. Flips to Accepted on the separate PO implementation-acceptance record.
